    ...
    def _create_anki_model(self):
        ...
        return genanki.Model(
            ...
            templates=[
                {
                    'name': 'Card 1',
                    'qfmt': '{{' + target_language_id + '}}<br><br>Baispal(e): {{' + target_language_examples_id + '}}',
                    'afmt': '{{FrontSide}}<hr id="answer">{{' + base_language_id + '}}<br><br>Beispiele: {{' + base_language_examples_id + '}}'
                },
                {
                    'name': 'Card 2',
                    'qfmt': '{{' + base_language_id + '}}<br><br>Beispiel(e): {{' + base_language_examples_id + '}}',
                    'afmt': '{{FrontSide}}<hr id="answer">{{' + target_language_id + '}}<br><br>Baispale: {{' + target_language_examples_id + '}}'
                }
            ],
            css=note_css
        )